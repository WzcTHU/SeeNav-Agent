...
class TrajectoryCollector:
    ...
    def vanilla_multi_turn_loop(
        ...
        for _step in range(self.config.env.max_steps):
            ...
            '''可以在这里添加特殊的reward定义，比如用于计算step_reward的信息，直接放到non_tensor_batch下面'''
            if 'step_reward' in infos[0]:
                batch.non_tensor_batch['raw_step_rewards'] = np.array([info['step_reward'] for info in infos], dtype=float)
    ...
    def multi_turn_loop(
        ...
        # print('######################### gen_batch_output @ rollout_loop multi_turn_loop start #########################')
        # print(gen_batch_output)
        # print('######################### gen_batch_output @ rollout_loop multi_turn_loop end #########################')
