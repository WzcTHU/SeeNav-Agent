...
class EBNavEnvironmentManager(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, config):
        self.memory = SimpleMemory()
        super().__init__(envs, projection_f, config)

    def __replace_none_with_first_non_none(self, lst, info):
        """
        将列表中的None值替换为列表中第一个非None值
        参数:
            lst: 包含可能含有None值的列表
        返回:
            替换后的新列表，如果原列表全是None则返回原列表
        """
        # 找到第一个非None值
        first_non_none = None
        for item in lst:
        ...
        # 如果没有找到非None值，直接返回原列表
        if first_non_none is None:
            print(f"############ {info} 中的值全为None ##########")
            return lst.copy()
        ...
        for item in lst:
            if item is None:
                new_list.append(first_non_none)
                none_count += 1
                print(f"############ {info} 中有{none_count}个None，替换为第一个非None值 ##########")
            else:
                new_list.append(item)
        
        return new_list
    
    def set_val_env_start_idx(self, val_idx):
        self.envs.val_start_idx = val_idx

    def reset(self, kwargs):
        text_obs, image_obs, infos = self.envs.reset()
        text_obs = self.__replace_none_with_first_non_none(text_obs, info='text_obs')
        image_obs = self.__replace_none_with_first_non_none(image_obs, info='image_obs')
        infos = self.__replace_none_with_first_non_none(infos, info='infos')
        self.memory.reset(batch_size=len(image_obs))
        self.pre_image_obs = image_obs
        observations = {
            'text': text_obs,
            'image': image_obs,
            'anchor': image_obs.copy()
        }
        return observations, infos
    ...
    def step(self, text_actions: List[str]):
        ...
        image_obs = self.__replace_none_with_first_non_none(image_obs, info='image_obs')
        infos = self.__replace_none_with_first_non_none(infos, info='infos')
        rewards = self.__replace_none_with_first_non_none(rewards, info='rewards')
        dones = self.__replace_none_with_first_non_none(dones, info='dones')

        for i, info in enumerate(infos):
            info['is_action_valid'] = to_numpy(valids[i])

        self.memory.store({'image_obs': self.pre_image_obs, 'action': actions})
        self.pre_image_obs = image_obs
        next_observations = {'text': text_obs, 'image': image_obs, 'anchor': image_obs.copy()}
        rewards = to_numpy(rewards)
        dones = to_numpy(dones)
        return next_observations, rewards, dones, infos
    ...
    def build_text_obs(self, text_obs: List[str]) -> List[str]:
        postprocess_text_obs = []
        for i in range(len(text_obs)):
            postprocess_text_obs.append(text_obs[i])
        return postprocess_text_obs
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        return envs, val_envs
    elif "ebnav" in config.env.env_name.lower():
        from agent_system.environments.env_package.ebnav import build_ebnav_envs, ebnav_projection, ebnav_projection_json
        ebnav_config_path = os.path.join(os.path.dirname(__file__), 'env_package/ebnav/configs/config_ebnav.yaml')
        _envs = build_ebnav_envs(ebnav_config_path, seed=config.env.seed, env_num=config.data.train_batch_size, group_n=group_n, resources_per_worker=resources_per_worker, is_train=True, config=config)
        _val_envs = build_ebnav_envs(ebnav_config_path, seed=config.env.seed + 1000, env_num=config.data.val_batch_size, group_n=1, resources_per_worker=resources_per_worker, is_train=False, config=config)

        if config.actor_rollout_ref.actor.output_format == 'json':
            projection_f = partial(ebnav_projection_json)
        else:
            projection_f = partial(ebnav_projection)
        envs = EBNavEnvironmentManager(_envs, projection_f, config)
        val_envs = EBNavEnvironmentManager(_val_envs, projection_f, config)
