...
class DataProto:
    ...
    #                 dtype = "Unknown"
    #                 device = "Unknown"
    #             lines.append(f"    - '{key}': shape={shape}, dtype={dtype}, type={ttype}, device={device}")
    #     # non_tensor_batch
    #     if not self.non_tensor_batch:
    #         lines.append("  non_tensor_batch: {}")
    #     else:
    #         lines.append("  non_tensor_batch (dict):")
    #         for key, val in self.non_tensor_batch.items():
    ...
    #                 shape = "Unknown"
    #                 dtype = "Unknown"
    #             lines.append(f"    - '{key}': shape={shape}, dtype={dtype}, type={vtype}")
    #     # meta_info
    #     if not self.meta_info:
    #         lines.append("  meta_info: {}")
    #     else:
    #         lines.append("  meta_info (dict):")
    #         for key, val in self.meta_info.items():
    #             vtype = type(val).__name__
    #             # shape/dtype only if it's an array/tensor
    #             if hasattr(val, 'shape'):
    #                 shape = tuple(val.shape)
    #             else:
    #                 shape = "-"
    #             lines.append(f"    - '{key}': type={vtype}, shape={shape}, value={repr(val)[:80]}")
    #     return "\n".join(lines)
    ...
    def __str__(self):
        ...
        if self.batch is None:
            ...
            for key in self.batch.keys():
                ...
                try:
                    if (
                        isinstance(tensor, (np.ndarray, torch.Tensor))
                        and hasattr(tensor, "shape")
                    ):
                        # if len(shape) == 1: # 1D
                        #     vals = tensor[:3]
                        #     details = f", values={vals.tolist() if isinstance(vals, torch.Tensor) else vals}"
                        # elif len(shape) == 2: # 2D
                        #     vals = tensor[:3, :4]
                        #     if isinstance(vals, torch.Tensor):
                        #         details = f", values={vals.tolist()}"
                        #     else:
                        #         details = f", values={vals}"
                        if len(shape) == 1: # 1D
                            head = tensor[:3]
                            tail = tensor[-3:]
                            details = f", values[:3]={head.tolist() if isinstance(head, torch.Tensor) else head}, values[-3:]={tail.tolist() if isinstance(tail, torch.Tensor) else tail}"
                        elif len(shape) == 2: # 2D
                            head = tensor[:3, :2]
                            tail = tensor[-3:, :2]
                            details = f", values[:3,:2]={head.tolist() if isinstance(head, torch.Tensor) else head}, values[-3:,:2]={tail.tolist() if isinstance(tail, torch.Tensor) else tail}"
                except Exception as e:
                    details = ""
                lines.append(
                    f"    - '{key}': shape={shape}, dtype={dtype}, type={ttype}, device={device}{details}"
                )
        # non_tensor_batch
        if not self.non_tensor_batch:
            lines.append("  non_tensor_batch: {}")
            ...
            for key, val in self.non_tensor_batch.items():
                ...
                try:
                    ...
                    details = ""
                lines.append(
                    f"    - '{key}': shape={shape}, dtype={dtype}, type={vtype}{details}"
                )
        # meta_info
        if not self.meta_info:
            lines.append("  meta_info: {}")
            ...
            for key, val in self.meta_info.items():
                ...
                try:
                    ...
                    details = ""
                lines.append(f"    - '{key}': type={vtype}, shape={shape}, value={repr(val):80}{details}")
        return "\n".join(lines)
