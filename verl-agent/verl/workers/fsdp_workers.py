...
class ActorRolloutRefWorker(Worker):
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            # print(f'######################## Len Data {len(data)} @ fsdp_workers update_actor 1 ######################')
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            # print(f'######################## Len Data {len(data)} @ fsdp_workers update_actor 2 ######################')
            # perform training
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if 'Qwen2.5-VL' in self.config.model.path:
                from verl.workers.critic import Qwen2_5_VLForConditionalGenerationWithValueHead
                critic_module = Qwen2_5_VLForConditionalGenerationWithValueHead.from_pretrained(
                    pretrained_model_name_or_path=local_path,
                    torch_dtype=torch_dtype,
