...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            # s
            # print(f'use_remove_padding: {self.use_remove_padding}')
            ...
            if self.use_remove_padding:
                ...
                # print('1 values_rmpad shape: ', values_rmpad.shape)

                values_rmpad = values_rmpad.squeeze(0)  # (total_nnz)

                # print('2 values_rmpad shape: ', values_rmpad.shape)
                # gather output if sp > 1
                if self.ulysses_sequence_parallel_size > 1:
                    values_rmpad = gather_outpus_and_unpad(values_rmpad, gather_dim=0, unpad_dim=0, padding_size=pad_size)
                # print('3 values_rmpad shape: ', values_rmpad.shape)
                # pad it back
                values = pad_input(values_rmpad, indices=indices, batch=batch, seqlen=seqlen).squeeze(-1)
                # print('4 values shape: ', values.shape)
                values = values[:, -response_length - 1 : -1]
                # print('5 values shape: ', values.shape)
            else:
                output = self.critic_module(
                    input_ids=input_ids,
    ...
    def compute_values(self, data: DataProto) -> torch.Tensor:
        ...
        # print('data.batch.batch_size[0]: ', data.batch.batch_size[0])
        # print('length of values list: ', len(values_lst))
        # print('values_lst[0].shape: ', values_lst[0].shape)
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                if has_multi_modal_inputs:
                    ...
                    micro_batches = data.select(select_keys, non_tensor_select_keys).chunk(num_micro_batches)
                    self.gradient_accumulation = self.config.ppo_mini_batch_size // self.config.ppo_micro_batch_size_per_gpu        # wzc add
